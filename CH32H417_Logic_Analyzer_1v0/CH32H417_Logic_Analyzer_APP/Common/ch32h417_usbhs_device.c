...
{
    USBHSD->UEP_TX_EN = USBHS_UEP0_T_EN | USBHS_UEP1_T_EN | USBHS_UEP2_T_EN | USBHS_UEP3_T_EN;
    USBHSD->UEP_RX_EN = USBHS_UEP0_R_EN | USBHS_UEP1_R_EN | USBHS_UEP2_R_EN | USBHS_UEP3_R_EN;
    ...
    USBHSD->UEP0_TX_CTRL = USBHS_UEP_T_RES_NAK; /*端点0发送初始化*/
    USBHSD->UEP0_RX_CTRL = USBHS_UEP_R_RES_ACK; /*端点0接收初始化*/

    USBHSD->UEP1_RX_CTRL = USBHS_UEP_R_RES_ACK; /*端点1接收初始化*/
    USBHSD->UEP2_RX_CTRL = USBHS_UEP_R_RES_ACK; /*端点2接收初始化*/
    USBHSD->UEP3_RX_CTRL = USBHS_UEP_R_RES_ACK; /*端点3接收初始化*/

    USBHSD->UEP1_TX_LEN = 0;
    USBHSD->UEP_TX_TOG_AUTO = USBHS_UEP1_T_EN;
    USBHSD->UEP_TX_BURST_MODE = USBHS_UEP1_T_EN; /*协议控制数据端点*/
    USBHSD->UEP_TX_BURST = USBHS_UEP1_T_EN;
    USBHSD->UEP1_TX_CTRL = USBHS_UEP_T_RES_NAK; /*端点1发送设置NAK*/

    USBHSD->UEP2_TX_LEN = 0;
    USBHSD->UEP_TX_TOG_AUTO |= USBHS_UEP2_T_EN;
    USBHSD->UEP_TX_BURST_MODE |= USBHS_UEP2_T_EN; /*逻辑分析发送数据端点*/
    USBHSD->UEP_TX_BURST |= USBHS_UEP2_T_EN;
    USBHSD->UEP2_TX_CTRL = USBHS_UEP_T_RES_NAK; /*端点2发送设置NAK*/

    USBHSD->UEP3_TX_LEN = 0;
    USBHSD->UEP_TX_TOG_AUTO |= USBHS_UEP3_T_EN;
    USBHSD->UEP_TX_BURST_MODE |= USBHS_UEP3_T_EN; /*ADC数据发送端点*/
    USBHSD->UEP_TX_BURST |= USBHS_UEP3_T_EN;
    USBHSD->UEP3_TX_CTRL = USBHS_UEP_T_RES_NAK; /*端点3发送设置NAK*/
}
    ...
    if(intflag & USBHS_UDIF_TRANSFER)
    {
        ...
        if(!(intst & USBHS_UDIS_EP_DIR))    // SETUP/OUT Transaction
        {
                ...
                if(USBHSD->UEP0_RX_CTRL & USBHS_UEP_R_SETUP_IS)
                {
                    ...
                    else
                    {
                        ...
                        case USB_SET_CONFIGURATION:
                            ...
                            logic_adc_info.usb_status = USB_U2_CONNECT;
                            USB_LINK_ON();
                            break;
            ...
            case DEF_UEP1:
                ...
                {
                    /* Write In Buffer */
                    logic_adc_info.usb20_endp1_down = 0x01;
        ...
        else
        {
            ...
            case DEF_UEP1:
                ...
                break;
            case DEF_UEP2:
                logic_adc_info.usb20_endp2_up = USB_SUCESEE_FLAG;
