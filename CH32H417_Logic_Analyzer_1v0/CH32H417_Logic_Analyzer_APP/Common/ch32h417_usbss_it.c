...
__attribute__ ((aligned(4))) uint8_t USBSS_EP1_Tx_Buf[ DEF_USB_EP1_SS_SIZE  ];     /* Endpoint 1 upload buffer size */
__attribute__ ((aligned(4))) uint8_t USBSS_EP1_Rx_Buf[ DEF_USB_EP1_SS_SIZE  ];     /* Endpoint 1 receive buffer size */

__attribute__ ((aligned(32),section(".da1"))) uint8_t USBSS_EP2_Tx_Th0_Buf[ DEF_USB_EP2_SS_SIZE*DEF_ENDP2_IN_BURST_LEVEL*DEF_UHSIF_TXBUF_CNT  ];     /* Endpoint 1 upload buffer size */
__attribute__ ((aligned(32),section(".da1"))) uint8_t USBSS_EP2_Tx_Th1_Buf[ DEF_USB_EP2_SS_SIZE*DEF_ENDP2_IN_BURST_LEVEL*DEF_UHSIF_TXBUF_CNT  ];     /* Endpoint 1 upload buffer size */
            ...
            switch( USBSS_SetupReqCode )
            {
                ...
                case USB_SET_CONFIGURATION:
                    ...
                    else
                    {
                        ...
                        logic_adc_info.usb_status = USB_U3_CONNECT;
                        USB_LINK_ON();
                    }
    ...
    else if( status & USBSS_UIF_TRANSFER )
    {
        ...
        if( ep_dir )
        {
            switch( endp_num )
            {
                ...
                case DEF_UEP1:
                    ...
                    break;
                case DEF_UEP2:      // Endpoint 2 is used for parallel port acquisition data transfer
                    USBSSD->EP2_TX.UEP_TX_CHAIN_ST |= USBSS_EP_TX_CHAIN_IF;
                    logic_adc_info.usb30_endp2_up = USB_SUCESEE_FLAG;
                break;
                case DEF_UEP3:      // Endpoint 3 is used for ADC acquisition data transfer
        ...
        else
        {
            switch( endp_num )
            {
                ...
                case DEF_UEP1:
                    logic_adc_info.usb30_endp1_down = 0x01;
